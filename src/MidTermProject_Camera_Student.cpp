...
/* MAIN PROGRAM */
// Got rid of int argc, const char *argv[]
int main()
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        vector<DataFrame>::iterator ring_buffer_start = dataBuffer.begin();
        if (dataBuffer.size() < dataBufferSize){
            dataBuffer.push_back(frame);
        }
        else{
            //Remove oldest image from stack and insert new one
            dataBuffer.erase(ring_buffer_start); 
            dataBuffer.push_back(frame); 
        ...
        bool rectangle_cropping = true;
        if (rectangle_cropping){
            cout << "CROPPING: Keypoints size before cropping: " << keypoints.size() << endl; 
            cv::Rect car_box = cv::Rect(535, 180, 180, 150); 
            ...
            for (size_t i = 0; i < keypoints.size(); ++i){
                if (car_box.contains(keypoints[i].pt)){
            ...
            cout << "Keypoints size without cropping: " << keypoints.size() << endl; 
        }
